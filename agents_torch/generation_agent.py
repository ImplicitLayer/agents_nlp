...
from base_agent import BaseAgent  # Абстрактный базовый агент


class TextGenerationAgent(BaseAgent):
    """
    Agent for text generation using a PyTorch model.
    """

    def __init__(self, model: nn.Module, tokenizer: PreTrainedTokenizer = None, device: str = None, max_length: int = 100):
        """
        Initializes the text generation agent with a PyTorch model and tokenizer.

        :param model: PyTorch model for text generation (e.g., GPT-2).
    ...
    def act(self, observation: str) -> str:
        """
        Generates text continuation based on the input prompt.

        :param observation: Input text (prompt).
    ...
    def train_step(self, observations, actions):
        """
        Fine-tunes the model using the given text generation data (teacher forcing).

        :param observations: List of input prompts (e.g., partial text).
        :param actions: List of expected outputs (e.g., full text).
